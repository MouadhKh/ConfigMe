...
import {Accordion, Button, Card, FormControl, FormControlProps, InputGroup, Modal} from "react-bootstrap";
...
import {RepositoryImportToast, toastOptions} from "../messages/toasts";
...
import {basicModeImportRepository} from "../../../../utils/basicModeUtils/BasicModeUtils";
...
export const ImportRepositoriesStep = () => {
    ...
    const importRepo = async (repositoryName: string, sourceUrl: string, azureToken: string, repositoryType: string) => {
        ...
        if (existingReposNames.includes(repositoryName) && !await isRepositoryEmpty(repositoryName, azureToken)) {
            ...
            return createRepository(organizationName, currentProjectId, repositoryName, azureToken, skip).then(createdRepo => {
    ...
    const deleteAndImportRepository = async (azureToken: string) => {
        ...
        await deleteRepository(organizationName, projectName, currentTargetRepositoryName,
            azureToken);
                                    ...
                                    <Accordion.Collapse eventKey="0">
                                        <Card.Body>
                                            <Button className="m-4" variant="outline-primary" onClick={async () => {
                                                const basicImportBaseRepoState = await basicModeImportRepository("Base-Container", "https://github.com/MouadhKh/Base-Container.git", azureCtx, repositoryDispatch, "BASE");
                                                const basicImportMainRepoState = await basicModeImportRepository(projectName, "https://github.com/MouadhKh/MainProject.git", azureCtx, repositoryDispatch, "MAIN");
